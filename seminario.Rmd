...
# Agrupar por Mes y calcular el promedio de las columnas numéricas
#calidad_agua_2020 <- datos_2020 %>%
#  group_by(Mes) %>%
#  summarise(across(where(is.numeric), mean, na.rm = TRUE))
...
data_clean <- datos_2020 %>%
  mutate(
    ...
    Calidad_Cloro = case_when(
      `Cloro Libre` <= 0.5 ~ "Aceptable",
      `Cloro Libre` > 0.5 & `Cloro Libre` <= 1.5 ~ "Moderada",
      `Cloro Libre` > 1.5 ~ "Alta",
      TRUE ~ "Desconocida"
    ),
    Calidad_Color = case_when(
      as.numeric(`Color Max: 15 Ud: pl/mg`) <= 15 ~ "Aceptable",
...
resumen_calidad <- data_clean %>%
  summarise(
    pH_Bueno = sum(Calidad_pH == "Buena", na.rm = TRUE),
    Turbidez_Optima = sum(Calidad_Turbidez == "Óptima", na.rm = TRUE),
    Color_Aceptable = sum(Calidad_Color == "Aceptable", na.rm = TRUE),
    Cloro_Aceptable = sum(Calidad_Cloro == "Aceptable", na.rm = TRUE),
    Olor_aceptable = sum(Calidad_Olor == "Buen estado", na.rm = TRUE),
    Sabor_aceptable = sum(Calidad_Sabor == "Recomendable", na.rm = TRUE),
    Temperatura_Aceptable = sum(Calidad_Temperatura == "Aceptable", na.rm = TRUE),
    Cloruros_Aceptables = sum(Calidad_Cloruros == "Aceptable", na.rm = TRUE),
    Fluor_Aceptable = sum(Calidad_Fluor == "Aceptable", na.rm = TRUE),
    Sodio_Aceptable = sum(Calidad_Sodio == "Aceptable", na.rm = TRUE),
    Aluminio_Aceptable = sum(Calidad_Aluminio == "Aceptable", na.rm = TRUE),
    Trihalometanos_Aceptables = sum(Calidad_Trihalometanos == "Aceptable", na.rm = TRUE),
    Oxidabilidad_Aceptable = sum(Calidad_Oxidabilidad == "Aceptable", na.rm = TRUE),
    Germenes_Aceptables = sum(Calidad_Germenes22C == "Aceptable", na.rm = TRUE),
    Clostridium_Aceptables = sum(Calidad_Clostridium == "Aceptable", na.rm = TRUE),
    Calcio_Bajo = sum(Calidad_Calcio == "Bajo", na.rm = TRUE),
    Magnesio_Bajo = sum(Calidad_Magnesio == "Bajo", na.rm = TRUE)
  )

# Visualizar el resumen
print(resumen_calidad)

# Guardar los datos limpios y clasificados
#write_csv(data_clean, "agua_calidad_clasificada_completa.csv")

# Crear un resumen de calidad por mes con columna "Aceptable/No Aceptable"
resumen_por_mes <- data_clean %>%
  group_by(Mes) %>%
  summarise(
    Calidad_pH = as.integer(any(Calidad_pH == "Buena", na.rm = TRUE)),
    Calidad_Turbidez = as.integer(any(Calidad_Turbidez == "Óptima", na.rm = TRUE)),
    Calidad_Cloro = as.integer(any(Calidad_Cloro == "Aceptable", na.rm = TRUE)),
    Calidad_Color = as.integer(any(Calidad_Color == "Aceptable", na.rm = TRUE)),
    Calidad_Olor = as.integer(any(Calidad_Olor == "Buen estado", na.rm = TRUE)),
    Calidad_Sabor = as.integer(any(Calidad_Sabor == "Recomendable", na.rm = TRUE)),
    Calidad_Temperatura = as.integer(any(Calidad_Temperatura == "Aceptable", na.rm = TRUE)),
    Calidad_Cloruros = as.integer(any(Calidad_Cloruros == "Aceptable", na.rm = TRUE)),
    Calidad_Fluor = as.integer(any(Calidad_Fluor == "Aceptable", na.rm = TRUE)),
    Calidad_Sodio = as.integer(any(Calidad_Sodio == "Aceptable", na.rm = TRUE)),
    Calidad_Aluminio = as.integer(any(Calidad_Aluminio == "Aceptable", na.rm = TRUE)),
    Calidad_Trihalometanos = as.integer(any(Calidad_Trihalometanos == "Aceptable", na.rm = TRUE)),
    Calidad_Oxidabilidad = as.integer(any(Calidad_Oxidabilidad == "Aceptable", na.rm = TRUE)),
    Calidad_Germenes22C = as.integer(any(Calidad_Germenes22C == "Aceptable", na.rm = TRUE)),
    Calidad_Clostridium = as.integer(any(Calidad_Clostridium == "Aceptable", na.rm = TRUE)),
    Calidad_Calcio = as.integer(any(Calidad_Calcio == "Bajo", na.rm = TRUE)),
    Calidad_Magnesio = as.integer(any(Calidad_Magnesio == "Bajo", na.rm = TRUE))
  ) %>%
  mutate(
    Aceptable = if_else(
      Calidad_pH == 1 & Calidad_Turbidez == 1 & Calidad_Cloro == 1 & Calidad_Color == 1 &
      Calidad_Olor == 1 & Calidad_Sabor == 1 & Calidad_Temperatura == 1 & Calidad_Cloruros == 1 &
      Calidad_Fluor == 1 & Calidad_Sodio == 1 & Calidad_Aluminio == 1 & Calidad_Trihalometanos == 1 &
      Calidad_Oxidabilidad == 1 & Calidad_Germenes22C == 1 & Calidad_Clostridium == 1 &
      Calidad_Calcio == 1 & Calidad_Magnesio == 1,
      "Aceptable", 
      "No Aceptable"
    )
  ) %>%
