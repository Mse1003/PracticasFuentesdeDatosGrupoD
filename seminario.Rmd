...
data_clean <- datos_2020 %>%
  mutate(
    ...
    Calidad_Color = case_when(
      `Color Max: 15 Ud: pl/mg` <= 15 ~ "Aceptable",
      `Color Max: 15 Ud: pl/mg` > 15 ~ "No Aceptable",
      TRUE ~ "Desconocida"
    ),
...
resumen_calidad <- data_clean %>%
  summarise(
    ...
    Olor_Sin = sum(Calidad_Olor == "Sin Olor", na.rm = TRUE),
    Sabor_Sin = sum(Calidad_Sabor == "Sin Sabor", na.rm = TRUE),
