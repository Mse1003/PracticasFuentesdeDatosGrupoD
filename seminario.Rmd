...
>>>>>>> baca0924586aa14eaadbfb24bc85aa8e0b330093
...
>>>>>>> a45298a4faa1b43b582c4d2b51c09be718dcba35
>>>>>>> baca0924586aa14eaadbfb24bc85aa8e0b330093
```

## Pregunta 1: ¿cual es la incidencia del tabaquismo en asturias?¿Hay relacion con el cancer de colon? En caso de haberla, ¿Como afecta dicha relacion?
```{r}
...
##DEMOSTRACION DE LA RELACION ENTRE EL TABAQUISMO Y EL CANCER DE COLON
...
tabla_combinada <- fumadores_asturias %>%
  ...
  mutate(
    CasosCancerColonPonderados = (PrevalenciaTabaquismo / 100) * CasosCancerColonTotal  
  )
...
## Pregunta 2: ¿cual es la incidencia del tabaquismo en asturias?¿Hay relacion con el cancer de colon? En caso de haberla, ¿Como afecta dihca relacion?
```{r}
#Ejercicio 2
library(readr)
library(dplyr)
...
#Creo la grafica, en este caso uno de barras sobre el cancer de colon
# Cargar librerías necesarias
library(ggplot2)
library(tidyr)
...
#Relacionado
# Convertir los datos de muertes y calidad de agua a formato largo
total_row_long <- total_row %>%
  pivot_longer(cols = everything(), names_to = "Mes", values_to = "Muertes")

# Asegurar que los nombres de los meses coincidan con los de resumen_por_mes
total_row_long <- total_row_long %>%
  mutate(Mes = toupper(Mes))
# Convertir Evaluación_final a factor si es necesario
resumen_por_mes <- resumen_por_mes %>%
  mutate(Evaluación_final = as.factor(Evaluación_final))
# Fusionar las tablas basándose en la columna `Mes`
merged_data <- resumen_por_mes %>%
  left_join(total_row_long, by = "Mes")
#Grafico
...
```
Ésta celda no es necesaria pero no borrar, pero no ejecutar
```{r}
...
```{r}
# Identificar columnas con variabilidad
...
# Calcular correlaciones solo para las columnas con variabilidad
