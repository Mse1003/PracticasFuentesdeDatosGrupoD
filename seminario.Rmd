...
```{r}
# Filtrar datos del agua solo para el año 2023 y reemplazar NA por 0
calidad_agua_2023 <- Agua_gijon %>%
  filter(Año == 2023) %>%  # Filtrar solo el año 2023
  mutate(across(everything(), ~ replace_na(., 0))) %>%  # Reemplazar todos los NA por 0
  group_by(Mes) %>%        # Agrupar por cada mes
  summarise(
    pH_promedio = mean(pH, na.rm = TRUE),
    turbidez_promedio = mean(Turbidez, na.rm = TRUE),
    nitratos_promedio = mean(Nitratos, na.rm = TRUE),
    bacterias_coliformes_promedio = mean(`Bacterías Coliformes`, na.rm = TRUE),
    enterococos_promedio = mean(Enterococos, na.rm = TRUE),
    trihalometanos_promedio = mean(`Trihalometanos`, na.rm = TRUE),
    fluor_promedio = mean(Fluor, na.rm = TRUE),
    cloruros_promedio = mean(Cloruros, na.rm = TRUE),
    dureza_promedio = mean(Dureza, na.rm = TRUE),
    sodio_promedio = mean(Sodio, na.rm = TRUE),
...
rm(tabla_completa)
# Reemplazar NA por 0 en las columnas seleccionadas
tabla_completa <- tabla_combinada %>%
  mutate(across(
    c(pH_promedio, nitratos_promedio, bacterias_coliformes_promedio, 
      enterococos_promedio, trihalometanos_promedio, fluor_promedio),
    ~ replace_na(., 0)  # Reemplazar NA por 0
  ))

# Calcular correlaciones después del reemplazo
correlaciones <- tabla_completa %>%
  summarise(
    pH = cor(Valor, pH_promedio),
    Nitratos = cor(Valor, nitratos_promedio),
    BacteriasColiformes = cor(Valor, bacterias_coliformes_promedio),
    Enterococos = cor(Valor, enterococos_promedio),
    Trihalometanos = cor(Valor, trihalometanos_promedio),
    Fluor = cor(Valor, fluor_promedio)
  )

# Ver las correlaciones calculadas
print(correlaciones)
